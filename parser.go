...
func (m *BinlogParser) GetBinLogData(element interface{}, e *canal.RowsEvent, n int) error {
	...
	for k := 0; k < num; k++ {
		...
		case "uint64":
			s.Field(k).SetUint(m.uintHelper(e, n, columnName))
		case "string":
...
func (m *BinlogParser) uintHelper(e *canal.RowsEvent, n int, columnName string) uint64 {
...
func (m *BinlogParser) boolHelper(e *canal.RowsEvent, n int, columnName string) bool {
	...
	val := m.uintHelper(e, n, columnName)
	if val == 1 {
		return true
	}
	return false
...
func (m *BinlogParser) decimalHelper(e *canal.RowsEvent, n int, columnName string) decimal.Decimal {
	columnId := m.getBinlogIdByName(e, columnName)
	if e.Table.Columns[columnId].Type != schema.TYPE_DECIMAL {
		panic("Not decimal type")
	}

	value, err := decimal.NewFromString(e.Rows[n][columnId].(string))
	if err != nil {
		panic("Cannot init decimal")
	}
